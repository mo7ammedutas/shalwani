...
| `AUTH_SECRET` | سر عشوائي (32+ بايت) لتوقيع جلسة لوحة التحكم — `node -e "console.log(require('crypto').randomBytes(32).toString('base64url'))"` |
...
الرابط: **`/ar/admin`** (أو `/en/admin`) — محمية بكلمة المرور من `ADMIN_PASSWORD`، والجلسة كوكي HTTP-only موقّع بـ HMAC صالح لأسبوع.

**الإمكانات:**
- **المنتجات**: إضافة/تعديل/حذف، الاسم والوصف بالعربي والإنجليزي، السعر بالريال (يُخزَّن بالبيسة تلقائياً)، المخزون، اللون ونوع التطريز، «منتج مميز» للرئيسية، ورفع صور متعددة (الأولى هي الرئيسية).
- **الحذف الذكي**: المنتج المرتبط بطلبات سابقة لا يُحذف (حفاظاً على سجل الطلبات) بل **يُؤرشف** ويختفي من المتجر فوراً، مع إمكانية إعادة نشره.
- **الطلبات**: جدول بآخر 100 طلب — الرقم، العميل، الهاتف، القطع، الإجمالي، حالة الدفع، التاريخ.

**ملاحظة للرفع على Vercel**: رفع الصور يكتب حالياً في `public/uploads/` وهذا يعمل محلياً وعلى خادم VPS، لكن نظام ملفات Vercel مؤقت — قبل الإطلاق هناك بدّل معالج الرفع (`src/app/api/admin/upload/route.ts`) إلى Vercel Blob أو S3، أو ارفع الصور عبر git في `public/products/`.
